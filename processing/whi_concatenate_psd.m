function [F, events, labels, classifiers, settings] = whi_concatenate_psd(files)
    ...
    for fId = 1:numfiles
        ...
        cdata = load(cfile, 'psd', 'events', 'settings', 'classifier', 'freqs');
...
%             end
        ...
        if (isequaln(csettings.spectrogram, settings.spectrogram) == false)
            keyboard
            error('chk:stn', ['Different spectrogram settings for file: ' cfile]);
        end
