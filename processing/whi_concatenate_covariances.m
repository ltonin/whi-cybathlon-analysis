function [C, events, labels, classifiers, settings] = whi_concatenate_covariances(files)
    ...
    NumFreqs   = unique(datasize(4, :));
    
    C  = nan(NumSamples, NumChans, NumChans, NumFreqs);
    ...
    for fId = 1:numfiles
        ...
        cfile = files{fId};
        whi_util_progress_display(fId, numfiles, '      ');
        cdata = load(cfile, 'C', 'events', 'settings', 'classifier');

        [maxpos, maxposid] = max(cdata.events.POS(cdata.events.TYP==800));
        cmaxdur = cdata.events.DUR(cdata.events.TYP==800);
        
        
        if maxpos + cmaxdur(maxposid) > size(cdata.C, 1)
            keyboard
        end
...
function dsizes = get_data_size(filepaths)
    ...
    ndimensions = 4;                            % samples x chans x chans x bands
    
    dsizes = zeros(ndimensions, nfiles);
    for fId = 1:nfiles
        cfilepath = filepaths{fId};
        cinfo = whos('-file', cfilepath, 'C');
